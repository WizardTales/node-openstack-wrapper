...
Keystone.prototype.getToken = function(username, password, domain, scope, cb) {
...
//make a callback(error, project_authorization) with all of the data on a project and an access token for further calls on it
//NOTE: this is not the admin function that gets project details - you have to do this so I'm not bothering with that
Keystone.prototype.getProjectTokenByName = function(
  access_token,
  domain_id,
  project_name,
  cb
...
Keystone.prototype.getProjectByName = function(
  admin_access_token,
  project_name,
  cb
) {
...
Keystone.prototype.listRoleAssignments = function(
  project_token,
  project_id,
  cb
) {
  ...
  this.request.get(request_options, function(error, response, body) {
    ...
    for (n = 0; n < body.role_assignments.length; n++) {
      assignments_array[n] = self.mangleObject(
        "RoleAssignment",
        body.role_assignments[n]
      );
    }
...
//NOTE: this is only works if the user is authed as an admin or projectAdmin
Keystone.prototype.addRoleAssignment = function(
  project_token,
  project_id,
  entry_id,
  entry_type,
...
//NOTE: this is only works if the user is authed as an admin or projectAdmin
Keystone.prototype.removeRoleAssignment = function(
  project_token,
  project_id,
  entry_id,
  entry_type,
...
Keystone.prototype.listMetaEnvironments = function(auth_token, cb) {
  ...
  this.request.get(request_options, function(error, response, body) {
    ...
    for (n = 0; n < body.environments.length; n++) {
      ...
      environments_array[n] = self.mangleObject(
        "MetaEnvironment",
        body.environments[n]
      );
    }
...
Keystone.prototype.listMetaOwningGroups = function(auth_token, cb) {
  ...
  this.request.get(request_options, function(error, response, body) {
    ...
    for (n = 0; n < body.owning_groups.length; n++) {
      //this is a little silly since its just id/name but meh...
      owning_groups_array[n] = self.mangleObject(
        "MetaOwningGroups",
        body.owning_groups[n]
      );
    }
...
//calls back with cb(error, meta_object)
Keystone.prototype.updateProjectMeta = function(
  project_token,
  project_id,
  new_meta,
  cb
...
Keystone.prototype.createProject = function(auth_token, project, cb) {
  var self = this;
  var request_options = {};
  ...
  request_options.metricPath = "remote-calls.keystone.projects.create";
  request_options.validateStatus = true;
  request_options.requireBodyObject = "project";
  ...
  this.request.post(request_options, function(error, response, body) {
    ...
    project_object = self.mangleObject("Project", body.project);
    cb(null, project_object);
  });
};
